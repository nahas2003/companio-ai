...
## 8. Database Status
* **Existing Tables:**
  - `User`: User profile table (`role` enum STUDENT/INSTRUCTOR/ADMIN/SUPER_ADMIN).
  - `Source`: Ingested study material files metadata.
  - `DocumentContent`: raw extracted plain text block (1-to-1 cascading relation to `Source`).
  - `AiUsageLog`: Stores providers, model, latency durations, success rates, and token counts.
  - `QuestionBank`: permanent named collections of generated questions.
  - `Question`: Question details mapping 1-to-many relationship with `QuestionBank`.
* **Database Sync:**
  - Pushed using `prisma db push` to synchronize remote schemas on Supabase PostgreSQL.
* **Missing Tables:**
...
* **Syncing profiles:** The frontend triggers `syncUser` Server Action upon successful login/register, creating/verifying records in the local `User` table.
...
* **Client-Side User Synchronization Risk:** Profiles are synced to PostgreSQL via client callbacks. If a user interrupts the session (e.g., closes tab) before the callback resolves, database syncing is incomplete.
* **Mock Statistics:** User dashboard statistics (Accuracy, answered questions) use mock responses from `dashboardService.ts`.
* **Direct Database Syncing:** `prisma db push` was used to sync database schemas instead of formal migration histories (`prisma migrate dev`).
* **Large Files text Ingestion limits:** Text extraction streams complete files into memory arrays. No chunking limits are set for handling extremely large textbooks (e.g., 50MB PDFs), which could cause memory issues in Server Actions.

---

## 15. Performance & Security Review
* **Performance:** Text parser utilities run completely inside server actions. For large documents (100+ pages), parsing may take over 10 seconds, which could block runtime executors.
* **Security & Auth:** Access token credentials are required in every Server Action. SQL transactions (`$transaction`) are utilized during write actions (such as saving decks to Question Bank) to ensure database integrity.
* **Storage Rules:** Supabase storage bucket `"sources"` is private and restricted, but there are no file-count limits per user.
...
* **Weaknesses:** Complete lack of automated unit tests, client-side user syncing risk, and hardcoded stats in dashboard logs.
* **Estimated Completion Percentage:** 55%
* **Production Readiness Score:** 50/100 (Incomplete practice/assessment engines, lack of automated tests, and missing migrations block production release).
