...
* **Separation of Concerns:** 
  ...
  * *Recommendation:* Move business-heavy orchestrations into separate service classes (e.g., `DocumentIngestionService`, `QuizGenerationService`) and keep Server Actions as lightweight request/response handlers.
* **Dependency Inversion Principle (DIP):**
  ...
  * *Recommendation:* Use a factory pattern or dependency injector to supply the orchestrator with active adapters.
* **Code Duplication:**
  ...
  * *Recommendation:* Consolidate JWT verification into a shared helper function in an auth utility file.
* **Reusability:**
  ...
  * *Recommendation:* Move these overlays into reusable modal components inside `packages/ui`.
...
* **Data Integrity & Constraints:**
  ...
  * *Recommendation:* Convert these to PostgreSQL Enums (`Status`, `QuestionType`, `Difficulty`) within the Prisma schema.
* **Missing Database Indexes:**
  * The schema does not define indexes on foreign keys, which will cause slow sequential table scans as records multiply.
  * *Recommendation:* Add the following indexes to `schema.prisma`:
...
* **Prisma Schema Sync vs. Migrations:**
  ...
  * *Recommendation:* Establish a migration baseline using `prisma migrate dev` before deploying to staging/production.
...
* **Hardcoded Prompt Templates:**
  ...
  * *Recommendation:* Keep templates in a database table (`PromptTemplate`) to support runtime editing and prompt A/B testing.
* **Retry Jitter:**
  ...
  * *Recommendation:* Add a random offset (jitter) to the backoff duration.
* **Caching Opportunities:**
  ...
  * *Recommendation:* Implement a key-value cache (e.g., using Redis) hashing `sourceId + type + difficulty + count` to immediately return previous results.
...
* **Client-Side Auth Token Dependency:**
  ...
  * *Recommendation:* Transition session management to HttpOnly session cookies verified by Next.js middleware.
* **File Upload Vulnerabilities:**
  ...
  * *Recommendation:* Enforce strict byte-size limits (e.g., max 10MB) and validate file headers during ingestion.
* **XSS Risks:**
  * Plain text parsed contents are saved raw in `DocumentContent` and output in preview fields.
  * *Recommendation:* Ensure appropriate sanitization before rendering AI outputs or extracted text.
* **Missing Rate Limiting:**
  ...
  * *Recommendation:* Implement rate-limiting middleware (e.g., token bucket algorithm) on all AI endpoints.
...
* **Memory Exhaustion on Large Files:**
  ...
  * *Recommendation:* Implement stream-based chunk parsing or delegate file processing to asynchronous background workers (e.g., BullMQ, Ingest).
* **React Render Cycles:**
  * In `/question-bank` and `/generate`, searching, checking boxes, or opening inline modals triggers parent component re-renders.
  * *Recommendation:* Memoize child elements and apply debouncing (e.g., 300ms) to search inputs to limit execution cascades.
* **Server Action Latency:**
  ...
  * *Recommendation:* Transition generation to asynchronous tasks that update client states via polling or server-sent events (SSE).
...
* **Accessibility (a11y):**
  ...
  * *Recommendation:* Audit interactive components for WCAG compliance.
* **Responsive Visual Scopes:**
  * The bulk operations bar and complex tables collapse on mobile screens, causing text truncation.
  * *Recommendation:* Implement card grid layouts for small screens rather than traditional HTML tables.
...
* **Hardcoded Model Names & Timeouts:**
  ...
  * *Recommendation:* Move these configurations into server-side environment variables or configuration constants.
* **User Synchronization Vulnerability:**
  ...
  * *Recommendation:* Use a Supabase Database Webhook trigger to automatically sync profiles at the database level during auth sign-up events.
